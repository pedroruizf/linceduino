...
void calculos() {
  ...
  if (botonpulsado == 1 && nvueltas > 2 && velocidad<100) {
    guardabuffer();
  }
}
...
void calculosmarcha() {
  ...
  if ((millis() - ultimamarcha) > 100 ) {
    marcha = !marcha;
    ultimamarcha = millis();
    if (botonpulsado == 1 && velocidad<100) {
      guardabuffer();
    }
  }
}
...
void loop() {
  ...
  lcd.setCursor(14, 1);
  //lcd.print (marcha);
  if ((velocidad > 100 || velocidadm > 100) && nvueltas > 2) {
    lcd.print("ERROR");
  }
  else {
    lcd.print("     ");
